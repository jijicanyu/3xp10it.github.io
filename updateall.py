import os
if os.path.exists('/root/myblog') is False and :
    print "this is the first time you use me,I will download blog_backup from https://github.com/3xp10it/blog_backup.git"
    os.system("mkdir /root/myblog && cd /root/myblog && git init && git pull https://github.com/3xp10it/blog_backup.git && git remote add origin https://github.com/3xp10it/blog_backup.git")
if os.path.exists('/root/3xp10it.github.io') is False:
...
os.system('''cd /root/3xp10it.github.io && cp -r ../myblog/_site/* . && git status && git add . && git status && git commit -a -m "update" && git push -u origin master && echo congratulations! commands execute ok to here!''')
